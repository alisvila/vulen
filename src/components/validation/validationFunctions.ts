...
export function checkIfStringIsGreaterThan(number: string | number | undefined, greater: number): boolean {
...
export function checkIfFilesAreImagePdfType(files: File | string): boolean {
  let valid = true;
  if (files && typeof files !== 'string') {
    if (!["image/gif", "image/jpeg", "image/png", "application/pdf"].includes(files.type)) {
      valid = false;
    }
  }
  return valid;
...
export function verifyIranianNationalId_2(nationalId?: string | number | null): boolean {
  ...
  console.log((sum < 2 && lastNumber === sum) || (sum >= 2 && lastNumber === 11 - sum), "heree")
	return (sum < 2 && lastNumber === sum) || (sum >= 2 && lastNumber === 11 - sum);
}


export function verifyIranianNationalId(nationalId?: string | number | null): boolean {
	if (!nationalId) return false;
    let national_code = nationalId.toString();
    if (!new RegExp('^\\d{10}$').test(national_code)) {
        return false
    }

    if (national_code === Array(national_code.length).fill(national_code[0]).join('')) {
        return false
    }

    let check = parseInt(national_code[9]);
    let partialNationalCodes = national_code.slice(0, 9).split('');
    let s = partialNationalCodes.map((el, i) => parseInt(el) * (10 - i)).reduce((a, b) => a + b, 0) % 11;
